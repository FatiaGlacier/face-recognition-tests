...
NUM_WORKERS = 8  # Або os.cpu_count()
CHUNK_SIZE = 20  # Фреймів на chunk
...
def process_chunk(frames_chunk, chunk_start_idx, chunk_id):
    """
    Обробляє один chunk фреймів з залоченим прогрес-баром
    """
    # Створюємо detector в worker
    with tqdm_lock:
        tqdm.write(f"Worker {chunk_id}: Ініціалізація MTCNN...")

    detector = MTCNN()
    chunk_results = []

    # Створюємо прогрес-бар з lock
    with tqdm_lock:
        pbar = tqdm(
            total=len(frames_chunk),
    ...
    for local_idx, frame in enumerate(frames_chunk):
...
def process_frames_multithreaded(frames, video_name):
    """
    ...
    with ThreadPoolExecutor(max_workers=NUM_WORKERS) as executor:
        ...
        for chunk_id, (chunk, start_idx) in enumerate(chunks):
            future = executor.submit(process_chunk, chunk, start_idx, chunk_id)
...
def process_all_videos_threading():
    ...
    for video_file in video_files:
        ...
        frame_data = process_frames_multithreaded(frames, video_name)
...
def get_frames_and_compare():
    FRAME_DIRs, video_names = process_all_videos_threading()
