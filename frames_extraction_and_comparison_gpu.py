...
CHUNK_SIZE = 30  # Фреймів на chunk
...
def process_frames(frames, detector, video_name):
    print(f"\n{'=' * 80}")
    print(f"📹 Обробка фреймів: {video_name}")
    print(f"{'=' * 80}")
    ...
    pbar = tqdm(total=len(frames), desc="Обробка фреймів")
...
def process_all_videos():
    ...
    for video_file in video_files:
        ...
        print(f"\n{'=' * 80}")
        print(f"\n COUNT: {count} | FRAMES: {len(frames)}")
        print(f"\n{'=' * 80}")

        frame_data = process_frames(frames, detector, video_name)
        print(f"\n{'=' * 80}")
        print(f"\n PROCESSED FRAMES: {len(frame_data)} ")
        print(f"\n{'=' * 80}")
